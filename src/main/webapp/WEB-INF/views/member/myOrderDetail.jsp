		...
		<section id="myOrderdetail">
			<div class="notice">
				<c:choose>
					...
					<c:when test="${map.O_STATUS  eq '2' }">
						<div class="inner">맛있게 만들고 있어요!<br>남은시간
						
						<span id="timer"></span>분</div>
					</c:when>
					<c:otherwise>
						<div class="inner">
			...
			<div class="pabout">
				<ul>
					...
					<li><strong>할인 금액</strong><span>
					<c:if test="${not empty map.coupon}">${map.coupon.CU_PRICE}</c:if><c:if test="${empty map.coupon}">0</c:if>원</span></li>
